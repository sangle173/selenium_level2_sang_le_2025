...
import com.testarchitect.framework.models.BillingFieldType;
...
public class CheckoutPage extends BasePage {
    ...
    private final SelenideElement checkoutForm = $(
            "form.checkout-form, .checkout-form, form.checkout, .checkout, .woocommerce-checkout, .checkout-page, form");
    private final SelenideElement orderSummary = $(
            ".order-summary, .order-details, .order-review, .checkout-summary, .cart-summary, .order-total");
    private final SelenideElement billingSection = $(
            ".billing-section, .billing-details, .billing-form, .checkout-billing");
    private final SelenideElement paymentSection = $(
            ".payment-section, .payment-methods, .payment-options, .checkout-payment");

    // Billing information - comprehensive field selectors
    private final SelenideElement firstNameField = $(
            "input[name='first_name'], input[name='billing_first_name'], input[name='firstName'], input[id*='first'], input[id*='fname']");
    private final SelenideElement lastNameField = $(
            "input[name='last_name'], input[name='billing_last_name'], input[name='lastName'], input[id*='last'], input[id*='lname']");
    private final SelenideElement emailField = $(
            "input[name='email'], input[name='billing_email'], input[type='email'], input[id*='email']");
    private final SelenideElement addressField = $(
            "input[name='address'], input[name='billing_address_1'], input[name='street_address'], input[id*='address']");
    private final SelenideElement address2Field = $(
            "input[name='address_2'], input[name='billing_address_2'], input[name='street_address_2'], input[id*='address2']");
    ...
    private final SelenideElement stateField = $(
            "select[name='state'], select[name='billing_state'], input[name='state'], input[name='billing_state'], input[id*='state']");
    private final SelenideElement zipField = $(
            "input[name='zip'], input[name='postal_code'], input[name='billing_postcode'], input[id*='zip'], input[id*='postal']");
    private final SelenideElement countryField = $(
            "select[name='country'], select[name='billing_country'], input[name='country'], input[id*='country']");
    private final SelenideElement phoneField = $(
            "input[name='phone'], input[name='billing_phone'], input[id*='phone']");

    // Payment methods
    private final SelenideElement creditCardOption = $(
            "input[value='credit-card'], input[value='stripe'], input[value='card'], input[type='radio'][value*='credit']");
    private final SelenideElement directBankOption = $(
            "input[value='direct-bank'], input[value='bacs'], input[value='bank'], input[type='radio'][value*='bank']");
    private final SelenideElement cashOnDeliveryOption = $(
            "input[value='cash-delivery'], input[value='cod'], input[value='cash'], input[type='radio'][value*='cash']");

    // Action buttons
    private final SelenideElement confirmOrderButton = $(
            "button.confirm-order, .confirm-order, button[name*='confirm'], input[value*='Confirm']");
    private final SelenideElement placeOrderButton = $(
            "button.place-order, .place-order, button[name*='place'], input[value*='Place'], button[name='woocommerce_checkout_place_order']");
    ...
    public CheckoutPage fillBillingDetails(BillingInfo billingInfo) {
        ...
        // Fill comprehensive billing information using the model
        try {
            // Process each billing field using switch case
    ...
    /**
     * Fill individual billing field based on field type
     * @param fieldType The type of field to fill
     * @param billingInfo The billing information model
     */
    private void fillBillingField(BillingFieldType fieldType, BillingInfo billingInfo) {
        try {
            switch (fieldType) {
                ...
                case EMAIL:
                    fillTextField(emailField, billingInfo.getEmail(), "email");
                    break;
                
                case ADDRESS:
                    fillTextField(addressField, billingInfo.getAddress(), "address");
                    break;
                
                case ADDRESS2:
                    fillTextField(address2Field, billingInfo.getAddress2(), "address 2");
                    break;
                
                case CITY:
                    fillTextField(cityField, billingInfo.getCity(), "city");
                    break;
                ...
                case ZIP:
                    fillTextField(zipField, billingInfo.getZip(), "zip");
                    break;
                
                case PHONE:
                    fillTextField(phoneField, billingInfo.getPhone(), "phone");
                    break;
                ...
                default:
                    logger.warn("Unknown billing field type: {}", fieldType);
                    break;
            }
        } catch (Exception e) {
    ...
    /**
     * Fill a regular text field
     * @param field The SelenideElement to fill
     ...
     */
    private void fillStateField(String state) {
        if (!stateField.exists() || state == null || state.trim().isEmpty()) {
            return;
        }

        try {
            switch (stateField.getTagName().toLowerCase()) {
                case "select":
                ...
                case "input":
                    ...
                    logger.info("Filled state field: {}", state);
                    break;
                
                default:
                    logger.warn("Unknown state field tag: {}", stateField.getTagName());
                    break;
            }
        } catch (Exception e) {
            logger.info("Could not fill state field: {}", e.getMessage());
        }
    }

    /**
     * Fill state select dropdown
     * @param state The state value to select
     */
    private void fillStateSelect(String state) {
        try {
            // Try different selection strategies
            if (stateField.$("option[value='" + state + "']").exists()) {
                stateField.selectOptionByValue(state);
                logger.info("Selected state by value: {}", state);
            } else if (stateField.$("option[text()='" + state + "']").exists()) {
                stateField.selectOption(state);
                logger.info("Selected state by text: {}", state);
            } else {
                // Fallback to California
                stateField.selectOption("California");
                logger.info("Selected fallback state: California");
            }
        } catch (Exception e) {
            logger.info("Could not select state: {}", e.getMessage());
        }
    }

    /**
     * Fill country field (handles both select and input)
     * @param country The country value to set
     */
    private void fillCountryField(String country) {
        if (!countryField.exists() || country == null || country.trim().isEmpty()) {
            return;
        }

        try {
            switch (countryField.getTagName().toLowerCase()) {
                case "select":
                ...
                case "input":
                    ...
                    logger.info("Filled country field: {}", country);
                    break;
                
                default:
                    logger.warn("Unknown country field tag: {}", countryField.getTagName());
                    break;
            }
        } catch (Exception e) {
     ...
     * @param country The country value to select
     */
    private void fillCountrySelect(String country) {
        try {
            // Try different selection strategies
            if (countryField.$("option[value='" + country + "']").exists()) {
                countryField.selectOptionByValue(country);
                logger.info("Selected country by value: {}", country);
                ...
                logger.info("Selected first available country option");
            }
        } catch (Exception e) {
            logger.info("Could not select country: {}", e.getMessage());
        }
    }

    @Step("Fill billing details with random data")
    public CheckoutPage fillBillingDetailsWithRandomData() {
        BillingInfo randomBillingInfo = BillingInfo.createRandom();
        return fillBillingDetails(randomBillingInfo);
    }

    @Step("Fill billing details with default payment method")
    public CheckoutPage fillBillingDetailsWithDefaultPayment() {
        BillingInfo defaultBillingInfo = BillingInfo.createDefault();
        fillBillingDetails(defaultBillingInfo);
        return selectDefaultPaymentMethod();
    }

    @Step("Fill billing details with custom data and default payment method")
    public CheckoutPage fillBillingDetailsWithCustomData(String firstName, String lastName, String email) {
        BillingInfo customBillingInfo = BillingInfo.createCustom(firstName, lastName, email);
        fillBillingDetails(customBillingInfo);
        return selectDefaultPaymentMethod();
    }

    @Step("Select default payment method")
    public CheckoutPage selectDefaultPaymentMethod() {
        logger.info("Selecting default payment method");
        handlePopups();
        ...
        try {
            ...
            try {
                paymentOption = $(
                        ".payment-method input[type='radio']:first-of-type, .payment-options input[type='radio']:first-of-type");
                if (paymentOption.exists()) {
                    paymentOption.click();
    ...
    @Step("Fill billing details with default payment method (Legacy)")
    public CheckoutPage fillBillingDetailsWithDefaultPaymentLegacy() {
